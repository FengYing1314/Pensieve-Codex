# Archived upstream v2 migration notes

These upstream notes are retained for historical context. Their version labels, migration commands, and descriptions of seed replacement or cleanup are not the current contracts for this fork. Use [the current lifecycle guide](../../.src/references/skill-lifecycle.md) and [the changelog](../../CHANGELOG.md) for supported behavior.

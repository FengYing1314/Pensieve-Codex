...
END {
    ...
    print ""
    print "> Auto-generated by Pensieve hooks or `maintain-project-state.sh`; run the command manually when hooks are unavailable."
    print ""
    print "- Root: `" root "`"
...
if [[ "$OUTPUT" == "-" ]]; then
    ...
    if [[ ${#files[@]} -eq 0 ]]; then
        run_awk < /dev/null > "$OUTPUT_TMP"
    else
        run_awk "${files[@]}" > "$OUTPUT_TMP"
    fi
    if [[ -f "$OUTPUT" ]] && cmp -s "$OUTPUT_TMP" "$OUTPUT"; then
        rm -f "$OUTPUT_TMP"
    else
